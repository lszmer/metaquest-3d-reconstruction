...
from app.dataset import DepthDatasetLoader, ColorDatasetLoader, RGBDDatasetLoader
...
class ProjectManager:
    def __init__(self, project_dir: Path):
        ...
        self.left_rgbd_dataset_loader = RGBDDatasetLoader(
            project_io_manager=self.io_manager,
            side=Side.LEFT,
            depth_dataset_loader=self.left_depth_dataset_loader,
            color_dataset_loader=self.left_color_dataset_loader,
        )
        self.right_rgbd_dataset_loader = RGBDDatasetLoader(
            project_io_manager=self.io_manager,
            side=Side.RIGHT,
            depth_dataset_loader=self.right_depth_dataset_loader,
            color_dataset_loader=self.right_color_dataset_loader,
        )
    ...
    def load_rgbd_dataset(self, side: Side):
        print(f"[Info] Loading RGBD dataset for {side} camera...")
        if side == Side.LEFT:
            return self.left_rgbd_dataset_loader.load_dataset()
        else:
            return self.right_rgbd_dataset_loader.load_dataset()
    ...
    def generate_point_cloud_tensor(
        ...
        print(f"[Info] Generating colorless point cloud with the following parameters:")
        ...
        if color:
            dataset = self.load_rgbd_dataset(side=Side.RIGHT)
            raise NotImplementedError("Color point cloud generation is not implemented yet.")
        else:
            depth_datasets = {
